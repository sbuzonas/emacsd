...
(defmacro defaddon (symbol description &rest forms)
  (progn
    ...
    (let ((hook-name (intern (format "after-%s-addon-hook" (fg/internal-addon-symbol symbol)))))
      (set hook-name '())
      (add-hook 'after-init-hook `(lambda ()
				    (let ((loading-message (format "Loading %s addon..." (symbol-name (fg/internal-addon-symbol ',symbol)))))
				      (message "%s" loading-message)
